...
function *startContainer(container) {
  ...
  var folder = path.join(__dirname, 'binds', container.id);
  yield mkdirp(folder);
  yield rimraf(folder + '/*');
...
function *unpauseContainer(container) {
  ...
  yield waitForPort(path.join(__dirname, 'binds', container.id));
}
...
function *waitForPort(folder) {
  ...
  while (count-- > 0) {
    try {
      var result = yield get({ socketPath: path.join(folder, 'proxy') });
      break;
    } catch (error) {
      ignore(error);
    }
